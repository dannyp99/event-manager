...
class EventController {
    ...
    constructor(){
        ...
        addButton.addEventListener('click', (event) => this.addEventButton())
        var deleteButton = document.getElementById('removeEvent')
        deleteButton.addEventListener('click', (event)=> this.deleteEventButton())
    }
    ...
    buildEventElement(event){
        ...
        eventContent.addEventListener('click', (event)=> {
            this.updateEventModifier(event.target)
        })
        return eventContent
    }
    ...
    updateEventModifier(eventSummary){
        let eventItems =  eventSummary.innerText.split(':')
        let htmlDate = this.helper.formatDate(eventItems[1])
